...
class ForgotPass extends Component {
  ...
  submitHandler = (event) => {
    ...
    const data = formData;
    axios
      .post("http://127.0.0.1:5050/user/forgotPassword", data, {
        withCredentials: true,
      })
      .then((response) => {
        if (this.mounted) {
          this.setState({ loading: false, reset: true, msg: "" });
        }
      })
      .catch((error) => {
          ...
          <div className="Contact">
            {this.state.reset ? (
              ...
              <div>
                ...
                {form}
                {!this.state.loading ? (
                  <Button btnType="Danger" clicked={this.navigateBack}>
                    Back
                  </Button>
                ) : null}
              </div>
