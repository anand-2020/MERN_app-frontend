...
class Users extends Component {
  ...
  componentDidMount() {
    this.mounted = true;
    axios
      .get("http://127.0.0.1:5050/user", { withCredentials: true })
      .then((response) => {
        if (this.mounted) {
          this.setState({ users: response.data.data.users, loading: false });
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  getPostHandler = (name) => {
    axios
      .get("http://127.0.0.1:5050/user/posts/" + name, {
        withCredentials: true,
      })
      .then((response) => {
        if (this.mounted) {
          this.setState({ posts: response.data.data.posts, display: true });
        }
      ...
      <Aux>
        ...
        (this.context.authenticated &&
          ...
          <div className="Users">
            {this.state.display ? (
              <div className="Posts">
                <button className="Cloj" onClick={this.changeDisplay}>
                  Close
                </button>
                {userPosts}{" "}
              </div>
